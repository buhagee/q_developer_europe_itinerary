...
// Parse the CSV data
const parseItinerary = (csvData: string) => {
...
if (!process.env.ITINERARY_TABLE || !process.env.PLACES_TABLE || !process.env.NOTES_TABLE) {
  console.error('Required environment variables are not set. Please set ITINERARY_TABLE, PLACES_TABLE, and NOTES_TABLE.');
  process.exit(1);
}
